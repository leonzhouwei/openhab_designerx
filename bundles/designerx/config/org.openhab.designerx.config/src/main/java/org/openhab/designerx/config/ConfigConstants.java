package org.openhab.designerx.config;

public final class ConfigConstants {

	// OS
	public static final String FILE_SEPARATOR = System.getProperty("file.separator");
	public static final String USER_DIR = System.getProperty("user.dir");
	
	// openHAB
	public static final String ADDONS = "addons";
	public static final String CONFIGURATIONS = "configurations";
	public static final String CONTEXTS = "contexts";
	public static final String DISTRIBUTION = "distribution";
	public static final String ETC = "etc";
	public static final String ITEMS = "items";
	public static final String LOGS = "logs";
	public static final String OPENHAB = "openhab";
	public static final String OPENHABHOME = "openhabhome";
	public static final String OPENHAB_RUNTIME = "openhab-runtime-";
	public static final String PERSISTENCE = "persistence";
	public static final String RULES = "rules";
	public static final String SCRIPTS = "scripts";
	public static final String SERVER = "server";
	public static final String SITEMAP = "sitemap";
	public static final String SITEMAPS = "sitemaps";
	public static final String SOUNDS = "sounds";
	public static final String TRANSFORM = "transform";
	public static final String WEBAPPS = "webapps";
	public static final String WORKSPACE = "workspace";
	
	//
	public static final String STRIKETHROUGH_80 = "--------------------------------------------------------------------------------";
}
